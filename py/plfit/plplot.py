...
def plplot(x,xmin,alpha):
    ...
    if f_dattype== 'REAL':
        ...
        q = sorted(list(filter(lambda X:X>=xmin,x)))
        ...
        c=[]
        for Q in q:
            c.append(len(list(filter(lambda X: floor(X)==Q,x)))/float(n))
        c1 = q+[q[-1]+1]
        c2 = map(lambda Z: 1.-Z,reduce(lambda X,Y: X+[Y+X[-1]],c,[0]))
        c2 = list(filter(lambda X:float(X)>=pow(10,-10.),c2))
        c1 = c1[0:len(c2)]
        ...
        cf2 = list(map(lambda Z: 1.-Z,reduce(lambda X,Y: X+[Y+X[-1]],cf,[0])))
        cf2 = list(map(lambda X: X*float(c2[c1.index(xmin)]),cf2))
